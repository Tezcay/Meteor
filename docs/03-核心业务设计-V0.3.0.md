...
日期：2026-09-16。配套：[需求与总体设计](01-项目需求与总体设计-V0.3.0.md)。  
状态：领域与行为设计，不是已实现代码、最终 DDL 或测试报告。英文名是模型用语，不要求直接成为界面文案。V0.3.0 沿用已有业务模型与状态机，具体 SQL 验证改以 MySQL／InnoDB 为准；Redis 展示缓存不参与权益正确性的判断。
